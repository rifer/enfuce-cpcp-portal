...
async function handleUpdateConfiguration(req, res, id) {
  ...
  if (updateData.card_scheme) {
    ...
    const schemeLower = updateData.card_scheme?.toLowerCase();
    ...
    if (schemeLower === 'visa' || schemeLower === 'v') {
      ...
      normalizedScheme = updateData.card_scheme?.charAt(0).toUpperCase() + updateData.card_scheme?.slice(1).toLowerCase();
    }
